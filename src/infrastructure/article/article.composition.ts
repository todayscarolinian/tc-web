...
export const articleService = {
  ...
  listTrending: (limit?: number) => listTrendingArticles(articleRepository, limit),
  search: (query: string) => searchArticles(articleRepository, query),
  listSections: () => articleRepository.listSections(),
  findSectionBySlug: (slug: string) => articleRepository.findSectionBySlug(slug),
  findSectionByName: (name: SectionName) => articleRepository.findSectionByName(name),
